...
- **个人中心**：设置 → 个人中心，Codex 风格的使用统计：单行统计条（累计 Token——输入/缓存命中/
  输出分桶、单日峰值含日期、最长会话持续时间、连续活跃天数，居中数值 + 标签、发丝分隔），
  GitHub 式 **Token 活动热力图**（全年 × 7 天，悬停看当日 token），以及按**真实模型名**聚合、
  按用量降序的**模型调用排行**（含占比；会话内切换模型时按请求时间线精确归因到各模型）。
